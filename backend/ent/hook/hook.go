...
// The RechargeLotteryDrawFunc type is an adapter to allow the use of ordinary
// function as RechargeLotteryDraw mutator.
type RechargeLotteryDrawFunc func(context.Context, *ent.RechargeLotteryDrawMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RechargeLotteryDrawFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.RechargeLotteryDrawMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RechargeLotteryDrawMutation", m)
}
