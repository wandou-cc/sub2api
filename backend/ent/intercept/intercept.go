...
// The RechargeLotteryDrawFunc type is an adapter to allow the use of ordinary function as a Querier.
type RechargeLotteryDrawFunc func(context.Context, *ent.RechargeLotteryDrawQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f RechargeLotteryDrawFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.RechargeLotteryDrawQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.RechargeLotteryDrawQuery", q)
}

// The TraverseRechargeLotteryDraw type is an adapter to allow the use of ordinary function as Traverser.
type TraverseRechargeLotteryDraw func(context.Context, *ent.RechargeLotteryDrawQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseRechargeLotteryDraw) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseRechargeLotteryDraw) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.RechargeLotteryDrawQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.RechargeLotteryDrawQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.RechargeLotteryDrawQuery:
		return &query[*ent.RechargeLotteryDrawQuery, predicate.RechargeLotteryDraw, rechargelotterydraw.OrderOption]{typ: ent.TypeRechargeLotteryDraw, tq: q}, nil
