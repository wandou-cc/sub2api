...
func (s *PaymentService) CreateOrder(ctx context.Context, req CreateOrderRequest) (*CreateOrderResponse, error) {
	...
	plan, carpoolPlan, err := s.validateOrderInput(ctx, req, cfg)
	if err != nil {
		return nil, err
	}
	carpoolTargetMembers := 0
	if carpoolPlan != nil {
		price, err := carpoolPricePerMember(carpoolPlan.TotalAmount, carpoolPlan.TargetMembers)
		if err != nil {
			return nil, infraerrors.BadRequest("INVALID_CARPOOL_PLAN", "carpool plan price is invalid")
		}
		req.Amount = price
		carpoolTargetMembers = carpoolPlan.TargetMembers
	}
	...
	if req.OrderType == payment.OrderTypeCarpool && methodCurrency != payment.DefaultPaymentCurrency {
		return nil, infraerrors.BadRequest("CARPOOL_CNY_ONLY", "carpool subscriptions require a CNY payment channel")
	}
	...
	if req.OrderType == payment.OrderTypeCarpool && selectedCurrency != payment.DefaultPaymentCurrency {
		return nil, infraerrors.BadRequest("CARPOOL_CNY_ONLY", "carpool subscriptions require a CNY payment channel")
	}
	...
	order, err := s.createOrderInTx(ctx, req, user, plan, carpoolPlan, cfg, orderAmount, limitAmount, feeRate, payAmount, sel)
	if err != nil {
		return nil, err
	}
	resp, err := s.invokeProvider(ctx, order, req, cfg, limitAmount, payAmountStr, payAmount, plan, carpoolTargetMembers, sel)
	if err != nil {
...
func (s *PaymentService) validateOrderInput(ctx context.Context, req CreateOrderRequest, cfg *PaymentConfig) (*dbent.SubscriptionPlan, *dbent.CarpoolPlan, error) {
	switch req.OrderType {
	case payment.OrderTypeSubscription:
		plan, err := s.validateSubOrder(ctx, req)
		return plan, nil, err
	case payment.OrderTypeCarpool:
		plan, err := s.validateCarpoolOrder(ctx, req)
		return nil, plan, err
...
func (s *PaymentService) createOrderInTx(ctx context.Context, req CreateOrderRequest, user *User, plan *dbent.SubscriptionPlan, carpoolPlan *dbent.CarpoolPlan, cfg *PaymentConfig, orderAmount, limitAmount, feeRate, payAmount float64, sel *payment.InstanceSelection) (*dbent.PaymentOrder, error) {
	...
	if carpoolPlan != nil {
		lockedPlan, err := tx.CarpoolPlan.Query().Where(carpoolplan.IDEQ(carpoolPlan.ID)).ForUpdate().Only(ctx)
		if dbent.IsNotFound(err) {
			return nil, infraerrors.Conflict("CARPOOL_PLAN_CHANGED", "carpool plan changed before order creation")
		}
		if err != nil {
			return nil, fmt.Errorf("lock carpool plan: %w", err)
		}
		if lockedPlan.Revision != carpoolPlan.Revision {
			return nil, infraerrors.Conflict("CARPOOL_PLAN_CHANGED", "carpool plan changed before order creation")
		}
		unfinished, err := tx.PaymentOrder.Query().Where(
			paymentorder.UserIDEQ(req.UserID),
			paymentorder.OrderTypeEQ(payment.OrderTypeCarpool),
			paymentorder.CarpoolPlanIDEQ(carpoolPlan.ID),
		...
		if err != nil {
			return nil, fmt.Errorf("check unfinished carpool order: %w", err)
		}
		if unfinished {
			return nil, infraerrors.Conflict("CARPOOL_PENDING_ORDER_EXISTS", "finish or cancel the pending carpool order first")
		}
		if lockedPlan.TargetMembers > 1 {
			joined, err := tx.CarpoolGroup.Query().Where(
				carpoolgroup.CarpoolPlanIDEQ(lockedPlan.ID),
				carpoolgroup.StatusEQ(CarpoolStatusWaiting),
				carpoolgroup.HasOrdersWith(paymentorder.UserIDEQ(req.UserID)),
			).Exist(ctx)
			if err != nil {
				return nil, fmt.Errorf("check current carpool membership: %w", err)
			}
			if joined {
				return nil, infraerrors.Conflict("CARPOOL_ALREADY_JOINED", "the user already has a seat in the current carpool group")
			}
		}
		carpoolPlan = lockedPlan
	}
	...
	if req.OrderType == payment.OrderTypeCarpool {
		b.SetCarpoolPlanID(carpoolPlan.ID).
			SetCarpoolPlanRevision(carpoolPlan.Revision).
			SetCarpoolSize(carpoolPlan.TargetMembers).
			SetCarpoolTotalAmount(carpoolPlan.TotalAmount).
			SetCarpoolPlanNote(carpoolPlan.Note)
	}
...
func (s *PaymentService) invokeProvider(ctx context.Context, order *dbent.PaymentOrder, req CreateOrderRequest, cfg *PaymentConfig, limitAmount float64, payAmountStr string, payAmount float64, plan *dbent.SubscriptionPlan, carpoolTargetMembers int, sel *payment.InstanceSelection) (*CreateOrderResponse, error) {
	...
	subject := s.buildPaymentSubject(plan, req.OrderType, carpoolTargetMembers, limitAmount, cfg, sel)
...
func (s *PaymentService) buildPaymentSubject(plan *dbent.SubscriptionPlan, orderType string, carpoolSize int, limitAmount float64, cfg *PaymentConfig, sel *payment.InstanceSelection) string {
	if plan != nil {
	...
	if orderType == payment.OrderTypeCarpool {
		return applyPaymentProductNameAffix(fmt.Sprintf("Sub2API Carpool Subscription %d-Person", carpoolSize), cfg)
	}
...
func (s *PaymentService) GetOrder(ctx context.Context, orderID, userID int64) (*dbent.PaymentOrder, error) {
	o, err := s.entClient.PaymentOrder.Query().Where(paymentorder.IDEQ(orderID)).WithCarpoolGroup().Only(ctx)
	if err != nil {
...
func (s *PaymentService) GetUserOrders(ctx context.Context, userID int64, p OrderListParams) ([]*dbent.PaymentOrder, int, error) {
	...
	orders, err := q.WithCarpoolGroup().Order(dbent.Desc(paymentorder.FieldCreatedAt)).Limit(ps).Offset((pg - 1) * ps).All(ctx)
	if err != nil {
