...
func (s *PaymentService) prepDeduct(ctx context.Context, o *dbent.PaymentOrder, p *RefundPlan, force bool) (*RefundResult, error) {
	...
	totalDeduction := p.RefundAmount + rewardDeduction
	if u.Balance < totalDeduction && !force {
		return &RefundResult{Success: false, Warning: "user balance is insufficient for deduction, use force", RequireForce: true}, nil
	}
	p.BalanceToDeduct = math.Max(0, math.Min(totalDeduction, u.Balance))
	return nil, nil
}
...
// finalizePendingRefundSuccess atomically claims and finalizes a confirmed pending refund.
func (s *PaymentService) finalizePendingRefundSuccess(ctx context.Context, p *RefundPlan) (_ *RefundResult, err error) {
	...
	if p.Order != nil && p.Order.OrderType == payment.OrderTypeCarpool && p.Order.CarpoolGroupID != nil {
		if syncErr := s.syncCarpoolRefundedGroups(ctx); syncErr != nil {
			slog.Error("failed to reconcile carpool refund status", "orderID", p.Order.ID, "error", syncErr)
		}
	}
	return result, nil
}
